...
const readOptions = program.readEncoding
  ? { encoding: program.readEncoding }
  : null;
const writeOptions = program.writeEncoding
  ? { encoding: program.writeEncoding }
  : null;
    ...
    files.forEach(file => {
      ...
      parse(content, parseOptions, function(err, trans) {
        ...
        while ((lang = langs.shift())) {
          ...
          fs.writeFileSync(
            target_file,
            JSON.stringify(buffer[lang], null, 2),
            writeOptions
          );
        }
      });
